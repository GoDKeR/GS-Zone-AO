...
Public Function AbrirComprimido(Optional ByVal Mensaje As String = "Seleccione el archivo Graficos.AO", Optional ByVal Filtro As String = "Graficos (*.AO)" & vbNullChar & "*.AO" & vbNullChar & "Todos los archivos" & vbNullChar & "*.*") As Boolean
...
Public Function GuardarComprimido(Optional ByVal Mensaje As String = "Indique donde será guardado Graficos.AO", Optional ByVal Filtro As String = "Graficos (*.AO)" & vbNullChar & "*.AO" & vbNullChar & "Todos los archivos" & vbNullChar & "*.*") As Boolean
On Error Resume Next
    ...
    If Not GetSaveFileName(OFName) = 0 Then
        sT = Split(Trim$(OFName.lpstrFile), Chr(0))(0)
        If InStr(1, Filtro, ".AO") <> 0 Then
            If UCase$(Right$(sT, 3)) <> ".AO" Then sT = sT & ".AO"
