...
On Error Resume Next
    ...
    Select Case sndRoute
        ...
        Case SendTarget.ToAdminsButCounselors ' 0.13.5
            For LoopC = 1 To LastUser
                With UserList(LoopC)
                    If .ConnID <> -1 Then
        ...
        Case SendTarget.ToAdminsButCounselorsAndRms ' 0.13.5
            For LoopC = 1 To LastUser
                With UserList(LoopC)
                    If .ConnID <> -1 Then
        ...
        Case SendTarget.ToHigherAdminsButRMs ' 0.13.5
            For LoopC = 1 To LastUser
                With UserList(LoopC)
                    If .ConnID <> -1 Then
        ...
        Case SendTarget.ToRMsAndHigherAdmins ' 0.13.5
            For LoopC = 1 To LastUser
                If (UserList(LoopC).ConnID <> -1) Then
...
'**************************************************************
    ...
    For LoopC = 1 To ConnGroups(Map).CountEntrys
        ...
        If UserList(tempIndex).AreasInfo.AreaReciveX And AreaX Then  'Esta en el area?
            If UserList(tempIndex).AreasInfo.AreaReciveY And AreaY Then
                If UserList(tempIndex).ConnIDValida Then
                    If UserList(tempIndex).flags.Privilegios Or (PlayerType.Consejero Or PlayerType.RoleMaster Or PlayerType.User) Then ' Estaba fallado? :S
                        Call EnviarDatosASlot(tempIndex, sdData)
                    End If
                End If
            End If
