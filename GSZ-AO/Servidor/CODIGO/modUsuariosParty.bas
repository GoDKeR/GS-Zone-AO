Attribute VB_Name = "modUsuariosParty"
'**************************************************************
' modUsuariosParty.bas - Library of functions to manipulate parties.
'
'**************************************************************
...
If PI > 0 Then
    ...
    End If
Else
    Call WriteMensajes(UserIndex, eMensajes.Mensaje080) '"No eres miembro de ninguna party."
End If
...
'*************************************************
    ...
    If PI > 0 Then
        ...
        Else
            Call WriteMensajes(User, eMensajes.Mensaje081) '"¡No eres el líder de tu party!"
            Exit Function
        End If
    Else
        Call WriteMensajes(User, eMensajes.Mensaje080) '"No eres miembro de ninguna party."
        Exit Function
    End If
End Function
    ...
    With UserList(NewMember)
        If .PartySolicitud = PI Then
            If Not .flags.Muerto = 1 Then
                If .PartyIndex = 0 Then
                    If Parties(PI).PuedeEntrar(NewMember, razon) Then
                        ...
                        Else
                            ...
                            Call SendData(SendTarget.ToAdmins, leader, PrepareMessageConsoleMsg(" Servidor> CATÁSTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S ", FontTypeNames.FONTTYPE_PARTY))
                            End If
                        Else
                        'no debe entrar
                ...
                End If
            Else
                Call WriteMensajes(leader, eMensajes.Mensaje082) '"¡Está muerto, no puedes aceptar miembros en ese estado!"
                Exit Sub
            End If
        Else
            If .PartyIndex = PI Then
