...
'***************************************************
    ...
    GUILDPATH = App.Path & "Guilds\" ' GSZAO
...
Dim GuildIndex  As Integer
    ...
    If Not m_EstadoPermiteEntrar(UserIndex, GuildIndex) Then
        ...
        Else
            ...
            Call LogClanes(UserList(UserIndex).Name & " de " & guilds(GuildIndex).GuildName & IIf(CambioAlineacion, " SI ", " NO ") & "provoca cambio de alineación. MAXANT:" & CambioAlineacion)
...
'14/01/2010: ZaMa - Pulo detalles en el funcionamiento modGeneral.
'***************************************************
    ...
    'If guilds(GuildIndex).CambiarAlineacion(BajarGrado(GuildIndex)) Then 'ALINEACION_NEUTRO)
        ...
        For MemberIndex = 0 To TotalMembers
            ...
            '[Silver - Sacar alineaciones de Clanes]
            'Tener en cuenta la variable "SALE" y su "IF" abajo, tambien comentado
            'If UserIndex > 0 Then
...
Public Sub ChangeCodexAndDesc(ByRef desc As String, ByRef codex() As String, ByVal GuildIndex As Integer, ByVal rLogo As String)
'***************************************************
'Author: Unknownn
'Last Modification: -
'
...
Public Function CrearNuevoClan(ByVal FundadorIndex As Integer, ByRef desc As String, ByRef GuildName As String, ByRef URL As String, ByRef codex() As String, ByRef refError As String, ByVal rLogo As String) As Boolean
'[Silver - Sacar alineaciones de Clanes]
'Public Function CrearNuevoClan(ByVal FundadorIndex As Integer, ByRef desc As String, ByRef GuildName As String, ByRef URL As String, ByRef codex() As String, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String, ByVal rLogo As String) As Boolean
'***************************************************
'Author: Unknownn
...
Dim DummyString     As String
    ...
    If CANTIDADDECLANES < UBound(guilds) Then
        ...
        If (iniCRequiereObj <> 0 And iniCRequiereObjCnt <> 0) Then ' GSZAO
            If TieneObjetos(iniCRequiereObj, iniCRequiereObjCnt, FundadorIndex) = False Then
                refError = "Para fundar un clan debes tener " & iniCRequiereObjCnt & " o más " & ObjData(iniCRequiereObj).Name & "."
                Exit Function
            Else
                Call QuitarObjetos(iniCRequiereObj, iniCRequiereObjCnt, FundadorIndex)
            End If
        End If
        ...
        With guilds(CANTIDADDECLANES)
            Call .Inicializar(GuildName, CANTIDADDECLANES)
            '[Silver - Sacar alineaciones de Clanes]
...
'***************************************************
'Author: Unknownn
'Last Modification: ^[GS]^- 30/07/2013
'
'***************************************************
...
Dim go As Integer
    ...
    If CANTIDADDECLANES = 0 Then ' GSZAO - No hay clanes
        UserList(UserIndex).GuildIndex = 0
        Exit Sub
    End If
    ...
    With guilds(GuildIndex)
        ...
        If .EleccionesAbiertas Then
            Call WriteMensajes(UserIndex, eMensajes.Mensaje094) '"Hoy es la votación para elegir un nuevo líder para el clan."
            Call WriteMensajes(UserIndex, eMensajes.Mensaje095) '"La elección durará 24 horas, se puede votar a cualquier miembro del clan."
            Call WriteMensajes(UserIndex, eMensajes.Mensaje096) '"Para votar escribe /VOTO NICKNAME."
            Call WriteMensajes(UserIndex, eMensajes.Mensaje097) '"Sólo se computará un voto por miembro. Tu voto no puede ser cambiado."
        End If
    End With
...
'***************************************************
    ...
    If UserList(UserIndex).Stats.ELV < iniCNivel Or UserList(UserIndex).Stats.UserSkills(eSkill.Liderazgo) < iniCLiderazgo Then
        refError = "Para fundar un clan debes ser Nivel " & iniCNivel & " y tener " & iniCLiderazgo & " skills en Liderazgo."
        Exit Function
    End If
    
    If (iniCRequiereObj <> 0 And iniCRequiereObjCnt <> 0) Then ' GSZAO
        If TieneObjetos(iniCRequiereObj, iniCRequiereObjCnt, UserIndex) = False Then
            refError = "Para fundar un clan debes tener " & iniCRequiereObjCnt & " o más " & ObjData(iniCRequiereObj).Name & "."
            Exit Function
        End If
    End If
...
'***************************************************
    ...
    '[Silver - Sacar alineaciones de Clanes]
    'Dejo la rutina porque se utiliza para enviar al cliente
    'y de esta forma no modifico el cliente(porque no estaba actualizado
    'al momento que realice estos cambios)
    Alineacion2String = ""
End Function
...
'***************************************************
    ...
    With guilds(GI)
        ...
        Call modProtocol.WriteGuildDetails(UserIndex, GuildName, .Fundador, .GetFechaFundacion, .GetLeader, .GetURL, .CantidadDeMiembros, .EleccionesAbiertas, Alineacion2String(.Alineacion), .CantidadEnemys, .CantidadAllies, .PuntosAntifaccion & "/" & CStr(MAXANTIFACCION), codex, .GetDesc, .GetLogo)
    End With
End Sub
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Call WriteGuildLeaderInfo(UserIndex, guildList, MemberList, guilds(GI).GetGuildNews(), aspirantsList, guilds(GI).GetLogo)
    End With
End Sub
...
'devuelve el guildindex
    ...
    Else
        Call WriteMensajes(UserIndex, eMensajes.Mensaje098) '"Error, el clan no existe."
        GMEscuchaClan = 0
    End If
...
On Error GoTo error
    ...
    If GI <= 0 Or GI > CANTIDADDECLANES Then
        Call modProtocol.WriteConsoleMsg(UserIndex, "No perteneces a ningún clan.", FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    End If
    
    If Not m_EsGuildLeader(UserList(UserIndex).Name, GI) Then
        Call modProtocol.WriteConsoleMsg(UserIndex, "No eres el líder de tu clan.", FontTypeNames.FONTTYPE_INFO)
        Exit Sub
    End If
    ...
    With UserFile
        ...
        Call modProtocol.WriteCharacterInfo(UserIndex, Personaje, .GetValue("INIT", "Raza"), .GetValue("INIT", "Clase"), .GetValue("INIT", "Genero"), .GetValue("STATS", "ELV"), .GetValue("STATS", "GLD"), .GetValue("STATS", "Banco"), .GetValue("REP", "Promedio"), .GetValue("GUILD", "Pedidos"), GuildName, Miembro, .GetValue("FACCIONES", "EjercitoReal"), .GetValue("FACCIONES", "EjercitoCaos"), .GetValue("FACCIONES", "CiudMatados"), .GetValue("FACCIONES", "CrimMatados"))
    End With
...
Dim AspiranteUI     As Integer
    ...
    If AspiranteUI > 0 Then
        ...
        If Not UserList(AspiranteUI).GuildIndex = 0 Then
    ...
    Else
        ...
        If GetGuildIndexFromChar(Aspirante) Then
            refError = Aspirante & " ya es parte de otro clan."
            Call guilds(GI).RetirarAspirante(Aspirante, NroAspirante)
            Exit Function
        End If
...
Public Sub SetNewGuildName(ByVal GuildIndex As Integer, ByRef newGuildName As String) ' 0.13.5
'***************************************************
'Author: Lex!
