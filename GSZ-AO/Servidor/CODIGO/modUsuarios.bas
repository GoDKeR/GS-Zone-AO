...
'*************************************************
    ...
    With UserList(UserIndex)
        ...
        If (.flags.Muerto = 1) Then
            GetNickColor = eNickColor.ieMuerto ' GSZAO color de Muerto!
        ElseIf EsNewbie(UserIndex) Then
            GetNickColor = eNickColor.ieNewbie ' GSZAO color de Newbie!
        Else
            If Criminal(UserIndex) Then
...
Public Function MakeUserChar(ByVal ToMap As Boolean, ByVal sndIndex As Integer, ByVal UserIndex As Integer, ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, Optional ButIndex As Boolean = False) As Boolean
'*************************************************
'Author: Unknownn
    ...
    With UserList(UserIndex)
        ...
        If InMapBounds(Map, X, Y) Then
            ...
            If Not ToMap Then
                ...
                Call WriteCharacterCreate(sndIndex, .Char.Body, .Char.Head, .Char.heading, .Char.CharIndex, X, Y, .Char.WeaponAnim, .Char.ShieldAnim, .Char.FX, 999, .Char.CascoAnim, UserName, NickColor, Privileges, 2)
            Else
...
On Error GoTo ErrHandler
    ...
    With UserList(UserIndex)
        Do While .Stats.Exp >= .Stats.ELU
            ...
            'Store it!
            Call modStatistics.UserLevelUp(UserIndex)
            
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_NIVEL, .Pos.X, .Pos.Y))
            Call WriteMensajes(UserIndex, eMensajes.Mensaje153) '"¡Has subido de nivel!"
            ...
            Else
                ...
                DistVida(1) = DistribucionEnteraVida(1) ' 0.13.3
                ...
                ElseIf aux <= DistVida(2) Then AumentoHP = Promedio + 1
                ElseIf aux <= DistVida(3) Then
                    AumentoHP = Promedio
            ...
            .Stats.MinHp = .Stats.MaxHp
                ...
                Call modUsuariosParty.ActualizarSumaNivelesElevados(UserIndex)
            ...
            If .Stats.ELV = 25 Then
                ...
                If GI > 0 Then
                    If modGuilds.GuildAlignment(GI) = "Del Mal" Or modGuilds.GuildAlignment(GI) = "Real" Then
                        ...
                        Call WriteMensajes(UserIndex, eMensajes.Mensaje154) '"¡Ya tienes la madurez suficiente como para decidir bajo que estandarte pelearás! Por esta razón, hasta tanto no te enlistes en la facción bajo la cual tu clan está alineado, estarás excluído del mismo."
                    End If
                End If
            End If
...
ErrHandler:
    ...
    Call LogError("Error en la subrutina CheckUserLevel - Error : " & Err.Number & _
        " - Description : " & Err.description & ". User: " & UserName & "(" & UserIndex & "). Map: " & UserMap) ' 0.13.5
...
'*************************************************
'Author: Unknownn
'Last Modification: 29/04/2012 - ^[GS]^
...
'***************************************************
    ...
    For LoopC = 1 To iniMaxUsuarios + 1
        If LoopC > iniMaxUsuarios Then Exit For
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .Invent.ArmourEqpObjIndex > 0 Then
            ...
            End If
        Else
            Call WriteMensajes(sendIndex, eMensajes.Mensaje156) '"(CUERPO) Mín Def/Máx Def: 0"
        End If
        ...
        Else
            Call WriteMensajes(sendIndex, eMensajes.Mensaje157) '"(CABEZA) Mín Def/Máx Def: 0"
        End If
        ...
        If GuildI > 0 Then
            ...
            If UCase$(modGuilds.GuildLeader(GuildI)) = UCase$(.Name) Then
                Call WriteMensajes(sendIndex, eMensajes.Mensaje158) '"Status: Líder"
            End If
            'guildpts no tienen objeto
        End If
        
        If .flags.Traveling = 1 Then ' 0.13.3
...
'*************************************************
    With UserList(UserIndex)
        ...
        ElseIf .fAccion.RecibioExpInicialReal = 1 Then
            Call WriteMensajes(sendIndex, eMensajes.Mensaje159) '"Fue ejército real"
            Call WriteConsoleMsg(sendIndex, "Veces que ingresó: " & .fAccion.Reenlistadas, FontTypeNames.FONTTYPE_INFO)
        
        ElseIf .fAccion.RecibioExpInicialCaos = 1 Then
            Call WriteMensajes(sendIndex, eMensajes.Mensaje160) '"Fue legión oscura"
            Call WriteConsoleMsg(sendIndex, "Veces que ingresó: " & .fAccion.Reenlistadas, FontTypeNames.FONTTYPE_INFO)
        End If
        ...
        Call WriteConsoleMsg(sendIndex, "Bandido: " & .Reputacion.BandidoRep, FontTypeNames.FONTTYPE_INFO) ' 0.13.5
...
'*************************************************
    ...
    If FileExist(CharFile) Then
        ...
        Call WriteConsoleMsg(sendIndex, "Bandido: " & CLng(GetVar(CharFile, "REP", "BANDIDO")), FontTypeNames.FONTTYPE_INFO) ' 0.13.5
        ...
        If .flags.Hambre = 0 And .flags.Sed = 0 Then
            ...
            With .Stats
                ...
                If .ExpSkills(Skill) >= .EluSkills(Skill) Then
                    ...
                    Call WriteMensajes(UserIndex, eMensajes.Mensaje162) '"¡Has ganado 50 puntos de experiencia!"
...
On Error GoTo ErrorHandler
    ...
    With UserList(UserIndex)
        ...
        Call SonidosMapas.ReproducirSonido(SendTarget.ToPCArea, UserIndex, iSoundDeath) ' 0.13.3
        ...
        If .PartyIndex > 0 Then
            Call modUsuariosParty.ObtenerExito(UserIndex, .Stats.ELV * -10 * modUsuariosParty.CantMiembros(UserIndex), .Pos.Map, .Pos.X, .Pos.Y)
        End If
        
        '<<Cerramos comercio seguro>>
        Call LimpiarComercioSeguro(UserIndex)
        
        ' Hay que teletransportar? ' 0.13.3
        Dim mapa As Integer
        mapa = .Pos.Map
        ...
        If MapaTelep <> 0 Then
            Call WriteMensajes(UserIndex, eMensajes.Mensaje456) ' "¡¡¡Tu estado no te permite permanecer en el mapa!!!"
            Call WarpUserChar(UserIndex, MapaTelep, MapInfo(mapa).OnDeathGoTo.X, _
                MapInfo(mapa).OnDeathGoTo.Y, True, True)
        End If

        UserDieAtTorneo UserIndex
...
'**************************************************************
    ...
    With UserList(UserIndex)
            ...
            If .flags.Privilegios And PlayerType.User Then 'El chequeo de invi/ocultar solo afecta a Usuarios (C4b3z0n)
                ...
                If AhoraVisible Then 'Si no era visible y ahora es, le avisa. (C4b3z0n)
                    ...
                    If WasInvi Then 'era invi
                        Call WriteMensajes(UserIndex, eMensajes.Mensaje457) ' "Has vuelto a ser visible ya que no esta permitida la invisibilidad en este mapa."
                    Else 'estaba oculto
                        Call WriteMensajes(UserIndex, eMensajes.Mensaje458) ' "Has vuelto a ser visible ya que no esta permitido ocultarse en este mapa."
                    End If
                End If
            End If
        ...
        If (.flags.Invisible = 1 Or .flags.Oculto = 1) And (Not .flags.AdminInvisible = 1) Then
            ...
            If .flags.Navegando = 0 Then ' 0.13.3
        ...
        If (.flags.Privilegios And (PlayerType.User)) = 0 Then ' 0.13.5
            If HayAgua(.Pos.Map, .Pos.X, .Pos.Y) Then
                If .flags.Navegando = 0 Then
                    .flags.Navegando = 1
...
'************************************************
    ...
    For i = 1 To MAXMASCOTAS
        ...
        If petType > 0 And canWarp Then
            ...
            If Index = 0 Then
                Call WriteMensajes(UserIndex, eMensajes.Mensaje163) '"Tus mascotas no pueden transitar este mapa."
            Else
    ...
    If InvocadosMatados > 0 Then
        Call WriteMensajes(UserIndex, eMensajes.Mensaje164) '"Pierdes el control de tus mascotas invocadas."
    End If
...
'************************************************
    ...
    With UserList(UserIndex)
        ...
        ' Exception: Pets don't spawn in water if they can't swim
        If NpcIndex = 0 Then
            Call WriteMensajes(UserIndex, eMensajes.Mensaje166) '"Tu mascota no pueden transitar este sector del mapa, intenta invocarla en otra parte."
        Else
...
'***************************************************
    ...
    With UserList(UserIndex)
        If .flags.UserLogged And Not .Counters.Saliendo Then
            ...
            .Counters.Salir = IIf((.flags.Privilegios And PlayerType.User) And MapInfo(.Pos.Map).Pk, Intervalos(eIntervalos.iCerrarConexion), 0)
            ...
            If isNotVisible Then
                ...
                If Not HiddenPirat Then Call WriteMensajes(UserIndex, eMensajes.Mensaje037) '"Has vuelto a ser visible."
...
'***************************************************
    If UserList(UserIndex).Counters.Saliendo Then
        ...
        If UserList(UserIndex).ConnIDValida Then
            ...
            Call WriteMensajes(UserIndex, eMensajes.Mensaje168) '"/salir cancelado."
        Else
            'Simply reset
            UserList(UserIndex).Counters.Salir = IIf((UserList(UserIndex).flags.Privilegios And PlayerType.User) And MapInfo(UserList(UserIndex).Pos.Map).Pk, Intervalos(eIntervalos.iCerrarConexion), 0)
        End If
    End If
End Sub
...
'***************************************************
    ...
    If FileExist(CharPath & Nombre & ".chr", vbArchive) = False Then
        Call WriteMensajes(sendIndex, eMensajes.Mensaje169) '"Pj Inexistente"
    Else
...
'***************************************************
    ...
    With UserList(UserIndex)
        If .flags.Muerto = 1 Then
            ...
            Tiempo = (Distance + 1) * MaximoInt((.Stats.ELV - 25), 0) * 1.5 ' [TEMPORAL]  30 'seg ' 0.13.5
        ...
        Else
            Call WriteMensajes(UserIndex, eMensajes.Mensaje170) ' "Debes estar muerto para poder utilizar este comando."
        End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If MapInfo(.Pos.Map).Pk = False Then
            Call WriteMensajes(UserIndex, eMensajes.Mensaje171) '"No puedes robar NPCs en zonas seguras."
            Exit Function
        End If
        ...
        If AtacablePor > 0 Then
            ...
            If StealingNpc Then
                ...
                If AtacablePor <> OwnerIndex Then
                    Call WriteMensajes(UserIndex, eMensajes.Mensaje172) '"No puedes atacar otra criatura con dueño hasta que haya terminado tu castigo."
                    Exit Function
                End If
...
Public Sub setHome(ByVal UserIndex As Integer, ByVal newHome As Byte, ByVal NpcIndex As Integer)
'***************************************************
'Author: Budi
'Last Modification: 07/09/2012 - ^[GS]^
'***************************************************

    If newHome < 1 Or newHome > NUMCIUDADES Then Exit Sub
...
Public Function GetHomeArrivalTime(ByVal UserIndex As Integer) As Long ' 0.13.5
'**************************************************************
'Author: ZaMa
'Last Modification: 27/07/2012 - ^[GS]^
...
Public Sub HomeArrival(ByVal UserIndex As Integer) ' 0.13.3
'**************************************************************
'Author: ZaMa
'Last Modification: 10/08/2011 - ^[GS]^
...
'---------------------------------------------------------------------------------------
' Procedure : PuedeDivorcio
' Author    : Facundo Ortega (GoDKeR)
' Date      : 23/12/2013
' Purpose   : Divorcio, para no vivir encadenado de por vida ^^
'---------------------------------------------------------------------------------------
'
Public Function PuedeDivorcio(ByVal oUserindex As Integer, ByVal dUserindex As Integer) As Boolean

    If oUserindex = dUserindex Then ' el mismo personaje
        Call WriteConsoleMsg(oUserindex, "No puedes casarte contigo mismo.", FontTypeNames.FONTTYPE_INFO)
        PuedeDivorcio = False
        Exit Function
    ElseIf LenB(UserList(oUserindex).flags.Matrimonio) = 0 Then ' que ya no este casado
        Call WriteConsoleMsg(oUserindex, "No estas casado.", FontTypeNames.FONTTYPE_INFO)
        PuedeDivorcio = False
        Exit Function
    ElseIf Not (StrComp(UserList(oUserindex).flags.Matrimonio, UserList(dUserindex).Name)) Then ' se divorcia con una persona que no se su esposa
        Call WriteConsoleMsg(oUserindex, UserList(dUserindex).Name & " no ha contraido matrimonio contigo.", FontTypeNames.FONTTYPE_INFO)
        PuedeDivorcio = False
        Exit Function
    ElseIf UserList(oUserindex).Pos.Map <> UserList(dUserindex).Pos.Map Then ' mismo mapa
        Call WriteConsoleMsg(oUserindex, UserList(dUserindex).Name & " se encuentra en un mapa distinto al tuyo.", FontTypeNames.FONTTYPE_INFO)
        PuedeDivorcio = False
        Exit Function
    ElseIf TieneObjInv(oUserindex, ObjMatrimonio1, 0, 0) = False Then ' tiene el acta de divorcio
        Call WriteConsoleMsg(oUserindex, "¡¡¿Dónde está el acta de divorcio?!!", FontTypeNames.FONTTYPE_WARNING)
        PuedeDivorcio = False
        Exit Function
    End If
...
Public Function PuedeCasarse(ByVal oUserindex As Integer, ByVal dUserindex As Integer) As Boolean ' GSZAO
'**************************************************************
'Author: ^[GS]^
...
'**************************************************************
    ...
    If oUserindex = dUserindex Then ' el mismo personaje
        ...
        PuedeCasarse = False
        Exit Function
    ElseIf UserList(oUserindex).Genero = UserList(dUserindex).Genero Then ' distinto genero
        Call WriteConsoleMsg(oUserindex, "No puedes contraer matrimonio con otro personaje de tu mismo género.", FontTypeNames.FONTTYPE_INFO)
        PuedeCasarse = False
        Exit Function
    ElseIf LenB(UserList(oUserindex).flags.Matrimonio) > 0 Then ' que ya no este casado
        Call WriteConsoleMsg(oUserindex, "Ya te encuentras en un matrimonio.", FontTypeNames.FONTTYPE_INFO)
        PuedeCasarse = False
        Exit Function
    ElseIf LenB(UserList(dUserindex).flags.Matrimonio) > 0 Then ' que con quien se quiere casar ya no lo este
        Call WriteConsoleMsg(oUserindex, UserList(dUserindex).Name & " ya se encuentra en un matrimonio.", FontTypeNames.FONTTYPE_INFO)
        PuedeCasarse = False
        Exit Function
    ElseIf UserList(oUserindex).Pos.Map <> UserList(dUserindex).Pos.Map Then ' mismo mapa
        Call WriteConsoleMsg(oUserindex, UserList(dUserindex).Name & " se encuentra en un mapa distinto al tuyo.", FontTypeNames.FONTTYPE_INFO)
        PuedeCasarse = False
        Exit Function
    ElseIf TieneObjInv(oUserindex, ObjMatrimonio2, 0, 0) = False Then ' tiene el anillo de compromiso
        'GODKER: GS, este no tendria que ser el dUserIndex? xD
        Call WriteConsoleMsg(oUserindex, "¡¡¿Dónde está el anillo de compromiso?!!", FontTypeNames.FONTTYPE_WARNING)
        PuedeCasarse = False
        Exit Function
    End If
...
Public Sub Enviar_HechizoAUser(ByVal Attacker As Integer, _
                               ByVal victim As Integer, _
        ...
        'Preparo el paquete.
        PackageToSend = PrepareMessageCreateParticle(ACharIndex, VCharIndex, EffectIndex, FXLoops)
        
        'Y envio :)
        SendData SendTarget.ToPCArea, victim, PackageToSend
End Sub

Public Sub Enviar_HechizoANpc(ByVal AttackerUser As Integer, _
                              ByVal VictimNpc As Integer, _
                              ByVal EffectIndex As Integer, _
                              ByVal FXLoops As Integer)
