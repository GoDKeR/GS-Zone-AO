...
'***************************************************
    ...
    Dim MostrarAyuda As Boolean ' GSZAO
    ...
    If LenB(Comando) = 0 Then Comando = Space$(1)
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/PING"
                If MainTimer.Check(TimersIndex.SendPing) Then ' GSZAO
                    Call WritePing
                Else
                    Call ShowConsoleMsg("No puedes hacer /PING tan rápido.")
                End If
            ...
            Case "/HORA"
                Call modProtocol.WriteServerTime
            ...
            Case "/ADMIN"
                If notNullArguments And CantidadArgumentos >= 2 Then
                    ...
                    Select Case UCase$(ArgumentosAll(1))
                        Case "+"
                            tmpInt2 = eAcciones.a_Agregar
                        Case "-"
                            tmpInt2 = eAcciones.a_Quitar
                        Case "="
                            tmpInt2 = eAcciones.a_Listar
                        Case Else
                            MostrarAyuda = True
                    End Select
                    If MostrarAyuda = False Then ' todo bien!
                        If tmpInt2 = eAcciones.a_Listar Then
                            Call WriteAdminCargos(tmpInt, tmpInt2, "")
                        Else
                            Call WriteAdminCargos(tmpInt, tmpInt2, ArgumentosAll(2))
                        End If
                    End If
                Else
                    MostrarAyuda = True
                End If
                If MostrarAyuda = True Then ' mostramos la ayuda si no sabe usar el comando ;)
                    Call ShowConsoleMsg("Syntaxis: /ADMIN <tipo> <accion> <nombre+del+usuario>")
                    Call ShowConsoleMsg("Los tipos pueden ser D(ios), S(emidios), C(onsejero) y R(olmaster).")
                    Call ShowConsoleMsg("Las acciones pueden ser +(agregar), -(quitar) y =(mostrar listado, no requiere ingresar un usuario).")
                End If
            ' GSZAO
            ...
            Case "/NOCAOS"
                If notNullArguments Then
                    tmpArr = Split(ArgumentosRaw, "@", 2) ' 0.13.5
                    If UBound(tmpArr) = 1 Then
            ...
            Case "/BUSCARNPC"
                ...
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /buscarnpc NOMBRE.")
                End If
            Case "/BUSCAROBJ"
                If notNullArguments Then
                ...
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Utilice /buscarobj NOMBRE.")
                End If
                
            Case "/LLUVIADEORO" ' GSZAO
                Call WriteLluviaDeORO
            ...
            Case "/ELIMINARPRETORIANOS" ' 0.13.3
            ...
            Case "/MODMAPINFO" ' PK, BACKUP
                If CantidadArgumentos > 1 Then
                    Select Case UCase$(ArgumentosAll(0))
                        ...
                        Case "ROBONPC" '/MODMAPINFO ROBONPC ' 0.13.3
                            Call WriteChangeMapInfoStealNpc(ArgumentosAll(1) = "1")
                            
                        Case "OCULTARSINEFECTO" '/MODMAPINFO OCULTARSINEFECTO ' 0.13.3
                            Call WriteChangeMapInfoNoOcultar(ArgumentosAll(1) = "1")
                            
                        Case "INVOCARSINEFECTO" '/MODMAPINFO INVOCARSINEFECTO ' 0.13.3
                            Call WriteChangeMapInfoNoInvocar(ArgumentosAll(1) = "1")
                                                
                    End Select
                Else
                    'Avisar que falta el parametro
                    Call ShowConsoleMsg("Faltan parámetros. Opciones: PK, BACKUP, RESTRINGIR, MAGIASINEFECTO, INVISINEFECTO, RESUSINEFECTO, TERRENO, ZONA, ROBONPC, OCULTARSINEFECTO, INVOCARSINEFECTO")
                End If
            ...
            Case "/IMPERSONAR" ' 0.13.3
                Call WriteImpersonate
                
            Case "/MIMETIZAR" ' 0.13.3
                Call WriteImitate
