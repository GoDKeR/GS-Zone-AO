...
Public Function Compress_Files(ByRef SourcePath As String, ByRef OutputPath As String, ByVal version As Long, ByRef prgBar As uAOProgress, Optional Mode As Byte = 0) As Boolean
'*****************************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
...
On Local Error GoTo ErrHandler
    ...
    Else
        SourceFileName = Dir(SourcePath & "*" & MAP_SOURCE_FILE_EXT, vbNormal) ' GSZAO
    End If
    ...
    If Mode = 0 Then  ' Comprimimos tambien los Graficos .PNG
    ...
    If Mode = 1 Then  ' agregamos tambien los BMP junto a los mapas
    ...
    If FileHead.lngNumFiles = 0 Then
        If Mode = 0 Then
            ...
            'End If
        Else
            MsgBox "No se encontraron archivos de extención " & MAP_SOURCE_FILE_EXT & " en " & SourcePath & ".", vbOKOnly + vbCritical, "Error"  ' GSZAO
        End If
        Exit Function
    End If
...
Public Function Extract_Files(ByRef ResourcePath As String, ByRef OutputPath As String, ByRef prgBar As uAOProgress) As Boolean
'*****************************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
'Last Modify Date: 16/07/2012 - ^[GS]^
'Extracts all files from a resource file
...
On Local Error GoTo ErrHandler
    ...
    For loopc = 0 To UBound(InfoHead)
        ...
        If Extract_File(ResourcePath, InfoHead(loopc), SourceData) Then
            'Destroy file if it previuosly existed
            If FileExists(OutputPath & InfoHead(loopc).strFileName) Then
...
Public Function Make_Patch(ByRef NewResourcePath As String, ByRef OldResourcePath As String, ByRef OutputPath As String, ByRef prgBar As uAOProgress) As Boolean
'*****************************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
'Last Modify Date: 08/24/2007
...
'
' @return   True if no error occurred.
Public Function Apply_Patch(ByRef ResourcePath As String, ByRef OldPath As String, ByRef PatchPath As String, ByRef prgBar As uAOProgress) As Boolean
'*****************************************************************
'Author: Nicolas Matias Gonzalez (NIGO)
'Last Modify Date: 08/24/2007
'Follows patches instructions to update a resource file
...
On Local Error GoTo ErrHandler
    ...
    Open sOldResourceFile For Binary Access Read Lock Write As ResourceFile
        ...
        Open sPatchFile For Binary Access Read Lock Write As PatchFile
            ...
            'Destroy file if it previuosly existed
            If FileExists(sNewResourceFile) Then Call Kill(sNewResourceFile)
